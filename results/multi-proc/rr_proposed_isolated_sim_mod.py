...
BREAK_MEAN = 1 / HOURS_TO_SECS(10)             # Param. for expovariate distribution
...
WEIBULL_SHAPE = 0.60
#WEIBULL_SHAPE = 1.0
WEIBULL_SCALE = HOURS_TO_SECS(10)/gamma(1.0+1.0/WEIBULL_SHAPE)
...
class BatchQueue(object):
    ...
    def preempt(self):
        ...
        start = 0
        while len(self.circQ) > 0 or (self.currentProc and self.currentProc.workLeft > 0):
            t = PT_MEAN / NUM_PROCESSES
            #if start == 0:
            #  t = int(PT_MEAN * LW_RUN_FRACTION / 100.0)
            #  start = 1
            #else:
            #  t = PT_MEAN - int(PT_MEAN * LW_RUN_FRACTION / 100.0)
...
def main(argc, argv, argList=None):
    ...
    parser.add_argument("--lw-run-fraction", type=float, help="Scaling factor for OCI of heavy application. Default is 1.0")
    if argList:
    ...
    if args.lw_run_fraction:
        LW_RUN_FRACTION = args.lw_run_fraction
