...
import rr_proposed_ckpt_aware_sim_mod as sim
...
for oci in range(int(sys.argv[2]), int(sys.argv[3])):
   ...
   with tqdm(total=RANGE*1) as pbar:
       for i in range(int(RANGE/(max_par_runs))):
           computeTime = 1000
           cmdSort = "-w -n 2 --run-time %d --sorted --mtbf 2 --ckpts-before-yield1 0 --ckpts-before-yield2 %d --oci-scale-factor %.1f" % (computeTime, lightAppCkpts, oci_factor)
