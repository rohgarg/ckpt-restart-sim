...
options(echo=FALSE) # don't want see commands in output file
args <- commandArgs(trailingOnly = TRUE)

if (length(args) < 5) {
 helpString = "Usage: Rscript model_prediction_components_two_jobs_version3.1.R <MAX_RUN_TIME> <BASE_DELTA> <DELTA_FACTOR> <MAX_SWITCH_POINT> <MTBF>\n"
 stop(helpString)
}

MAX_RUN_TIME <- as.integer(args[1]) # as.Date(args[1])
BASE_DELTA <- as.double(args[2])
DELTA_FACTOR <- as.integer(args[3])
MAX_SWITCH_POINT <- as.integer(args[4])
MTBF <- as.double(args[5])
rm(args)
...
cat("\n------------------Switching Results: Switch Point ", i, "---------------------")
cat("\nSwitch Point ", i)
